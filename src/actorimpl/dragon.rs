...
impl Actor for Dragon {
    fn pre_start(&self, context: ActorCell) {
        let wings = context
            .actor_of(Props::new(Arc::new(Wings::new), ()), "wings".to_owned())
            .unwrap();
        let mouth = context
            .actor_of(Props::new(Arc::new(Mouth::new), ()), "mouth".to_owned())
            .unwrap();
        let eyes = context
            .actor_of(Props::new(Arc::new(Eyes::new), ()), "eyes".to_owned())
            .unwrap();
        ...
        if let Ok(_message) = Box::<Any>::downcast::<DragonCommands>(_message) {
            match *_message {
                DragonCommands::MoveWings => {
                    let wings: ActorRef = _context
                        .children()
                        .get(&ActorPath::new_local("/user/gorynich/wings".to_owned()))
                        .cloned()
                        .unwrap();
                ...
                DragonCommands::OpenMouth => {
                    let mouth: ActorRef = _context
                        .children()
                        .get(&ActorPath::new_local("/user/gorynich/mouth".to_owned()))
                        .cloned()
                        .unwrap();
                ...
                DragonCommands::BlinkEyes => {
                    let eyes: ActorRef = _context
                        .children()
                        .get(&ActorPath::new_local("/user/gorynich/eyes".to_owned()))
                        .cloned()
                        .unwrap();
...
impl Actor for Wings {
    fn pre_start(&self, context: ActorCell) {
        context
            .actor_of(Props::new(Arc::new(PwmActor::new), 0), "pwm18".to_owned())
            .unwrap();
    }
    fn receive(&self, _message: Box<Any>, _context: ActorCell) {
        if let Ok(_message) = Box::<Any>::downcast::<LimbCommands>(_message) {
            match *_message {
                LimbCommands::Init(max) => {
                    println!("Initializing with maximum {}", max);
                }
                LimbCommands::Aggitate => {
                    println!("Moving Wings");
                    let pwm18: ActorRef = _context
                        .children()
                        .get(&ActorPath::new_local(
                            "/user/gorynich/wings/pwm18".to_owned(),
                        ))
                        .cloned()
                        .unwrap();
...
impl Actor for Eyes {
    fn pre_start(&self, context: ActorCell) {
        context
            .actor_of(Props::new(Arc::new(PinActor::new), 22), "pin22".to_owned())
            .unwrap();
    }
        ...
        if let Ok(_message) = Box::<Any>::downcast::<LimbCommands>(_message) {
            match *_message {
                ...
                LimbCommands::Aggitate => {
                    ...
                    let pin22: ActorRef = _context
                        .children()
                        .get(&ActorPath::new_local(
                            "/user/gorynich/eyes/pin22".to_owned(),
                        ))
                        .cloned()
                        .unwrap();
